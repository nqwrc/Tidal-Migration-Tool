...
class TidalMigratorGUI:
    ...
    def __init__(self, migrator):
        ...
        self.root = tk.Tk()
        self.root.title("Tidal Migrator")
        self.root.geometry('250x282')
        ...
        self.login_button = tk.Button(main_frame, text="Login", command=self.on_login_button_click)
        ...
        self.upload_button = tk.Button(main_frame, text="Upload CSV to Tidal", command=self.on_upload_button_click)
        self.upload_button.grid(row=2, column=0, columnspan=2, sticky="ew", pady=(5, 10))

        favorites_frame = tk.LabelFrame(main_frame, text="Favorites", padx=10, pady=10)
    ...
    def check_login_status(self):
        ...
        if self.migrator.check_login():
            self.status_label.config(text="Logged in as " + self.migrator.session.user.email, fg='green')
    ...
    def on_upload_button_click(self):
        ...
        errors = self.migrator.upload_favorites(filepaths)
        if errors:
